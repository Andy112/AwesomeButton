package mad.acai.com.awesomebutton.tabs;
...
/**
 * A Toolbar that contains multiple mad.acai.com.awesomebutton.tabs
 * @author neokree
 *
 */
...
public class MaterialTabHost extends RelativeLayout implements View.OnClickListener {
    ...
    private boolean isStyleBold;
	private List<mad.acai.com.awesomebutton.tabs.MaterialTab> tabs;
	...
	public MaterialTabHost(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// initialize mad.acai.com.awesomebutton.tabs list
		tabs = new LinkedList<mad.acai.com.awesomebutton.tabs.MaterialTab>();
	...
	public void setPrimaryColor(int color) {
		...
		for(mad.acai.com.awesomebutton.tabs.MaterialTab tab : tabs) {
    ...
    public void setStyleBold(boolean isStyleBold) {
        this.isStyleBold = isStyleBold;
        if (isStyleBold) {
            for(mad.acai.com.awesomebutton.tabs.MaterialTab tab : tabs) {
                tab.setTextStyle(isStyleBold);
            }
        }
    }
	...
	public void addTab(mad.acai.com.awesomebutton.tabs.MaterialTab tab) {
        ...
        tab.setTextStyle(true);

        // insert new tab in list
        tabs.add(tab);

        if(tabs.size() == 4 && !hasIcons) {
            // switch mad.acai.com.awesomebutton.tabs to scrollable before its draw
            scrollable = true;
        }
	...
	public mad.acai.com.awesomebutton.tabs.MaterialTab newTab() {
		return new mad.acai.com.awesomebutton.tabs.MaterialTab(this.getContext(),hasIcons);
	}
	...
	public void setSelectedNavigationItem(int position) {
		if(position < 0 || position > tabs.size()) {
			...
			for(int i = 0; i < tabs.size(); i++) {
				mad.acai.com.awesomebutton.tabs.MaterialTab tab = tabs.get(i);
    ...
    public void notifyDataSetChanged() {
        ...
        if (!scrollable) { // not scrollable mad.acai.com.awesomebutton.tabs
            int tabWidth = this.getWidth() / tabs.size();

            // set params for resizing mad.acai.com.awesomebutton.tabs width
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(tabWidth, HorizontalScrollView.LayoutParams.MATCH_PARENT);
            for (mad.acai.com.awesomebutton.tabs.MaterialTab t : tabs) {
                layout.addView(t.getView(), params);
            }

        } else { //scrollable mad.acai.com.awesomebutton.tabs
        ...
        if (isTablet && scrollable) {
            // if device is a tablet and have scrollable mad.acai.com.awesomebutton.tabs add right and left arrows
    ...
    public mad.acai.com.awesomebutton.tabs.MaterialTab getCurrentTab() {
        for(mad.acai.com.awesomebutton.tabs.MaterialTab tab : tabs) {
            if (tab.isSelected())
                return tab;
        }
